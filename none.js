    ...
    connection.query(checkQuery, [address, house], (err, results) => {
        ...
        if (results.length === 0) {
            ...
            connection.query(insertQuery, [address, house, housetype, housename, latestactivity], (err) => {
                ...
                // Now update the total mint for the house (collection)
                const updateHouseQuery = 'UPDATE taikocampaign SET totalmint = totalmint + 1 WHERE house = ?';
                connection.query(updateHouseQuery, [house], (err) => {
                    if (err) {
                        console.error('Error updating house mint count:', err.stack);
                        return res.status(500).json({ error: 'Internal server error while updating house' });
                    }
                    console.log(`House mint count updated for: ${house}`);
                    res.status(200).json({ message: 'Player details saved and house mint count updated successfully' });
                });
            });
        } else {
            ...
            connection.query(updateQuery, [latestactivity, address, house], (err) => {
                ...
                // Also update the total mint for the house (collection)
                const updateHouseQuery = 'UPDATE taikocampaign SET totalmint = totalmint + 1 WHERE house = ?';
                connection.query(updateHouseQuery, [house], (err) => {
                    if (err) {
                        console.error('Error updating house mint count:', err.stack);
                    ...
                    console.log(`House mint count incremented for: ${house}`);
                    res.status(200).json({ message: 'TotalMint incremented and house mint count updated successfully' });
                });
            });
        }
    });
